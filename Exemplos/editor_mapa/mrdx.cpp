...
COMENTÁRIOS:
				...
				Em homenagem aos 10 anos do MRDX eu criei a versão 1.1 com algumas 
				funcionalidades novas que se encontram no final deste arquivo.
...
int Destruir_Imagem(IMAGEM &imagem)
{
	...
	if(imagem.clone) 
	{
		//anula as referencias usadas pelo clone
	  for (i=0; i<imagem.num_quadros; i++)
            imagem.quadros[i] = NULL;
...
int Carregar_Quadro(IMAGEM &imagem, int quadro, int cx, int cy, bool borda, int origemX, int origemY)
{
	// extrai um quadro de um bitmap
	unsigned char *ptr_font, *ptr_dest;

	DDSURFACEDESC2 ddsd;  

	if(borda) 
	{
		//converte coordenadas adicionando 1 pixel para cada borda
		cx = cx* (imagem.largura + 1) + 1;
		cy = cy* (imagem.altura + 1) + 1;
	} 
	else 
	{
	...
	//levar em conta nova origem caso haja
	cx += origemX;
	cy += origemY;
...
//////////////////////////////////////////////
int Desenhar_Imagem(IMAGEM &imagem, int espelhar, int escalaX , int escalaY )
{
	...
	{
	 ret_dest.right  = imagem.x + escalaX; 
	 ret_dest.bottom = imagem.y + escalaY; 
	} 
	else 
	{
	...
	DWORD dwFlags = DDBLT_WAIT | DDBLT_KEYSRC;	
...
int Clone_Imagem(IMAGEM &imagem_fonte, IMAGEM &imagem_destino)
{
   ...
   if (&imagem_fonte != &imagem_destino)
   {
   
     memcpy(&imagem_destino, &imagem_fonte, sizeof(IMAGEM));
     
	 imagem_destino.clone = true;
...
int Transicao_Tela(int cor) {
	...
	if(cor<0 || cor>255) return 0;

	PALETTEENTRY cor_temp;              
	PALETTEENTRY cor_destino;
	PALETTEENTRY palheta_temp[256];  

	int pal_reg; 

	memcpy(palheta_temp, palheta,256*sizeof(PALETTEENTRY));

	cor_destino = palheta_temp[cor];
           ...
           {               
               ...
               for (pal_reg=0; pal_reg<256; pal_reg++)
               {
                   ...
                   cor_temp = palheta_temp[pal_reg];

                   
                   if (cor_temp.peRed > cor_destino.peRed + 4 ) cor_temp.peRed -=4;
				   else if (cor_temp.peRed < cor_destino.peRed - 4 ) cor_temp.peRed +=4;
                   else
                      cor_temp.peRed = cor_destino.peRed;

                   if (cor_temp.peGreen > cor_destino.peGreen + 4) cor_temp.peGreen -=4;
				   else if (cor_temp.peGreen < cor_destino.peGreen - 4) cor_temp.peGreen +=4;
                   else
                      cor_temp.peGreen = cor_destino.peGreen;

                   if (cor_temp.peBlue  > cor_destino.peBlue + 4) cor_temp.peBlue -=4;
				   else if (cor_temp.peBlue  < cor_destino.peBlue - 4) cor_temp.peBlue +=4;
                   else
                      cor_temp.peBlue = cor_destino.peBlue;

                   
                   palheta_temp[pal_reg] = cor_temp;

               } 

               
				lpddpal->SetEntries(0,0,256,palheta_temp);
		   ...
		   Esperar_Relogio(50);
			...
			//restaurar palheta	original   
			lpddpal->SetEntries(0,0,256,palheta);
